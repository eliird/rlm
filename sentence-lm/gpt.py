    ...
    def forward(self, x, attn_mask=None):
        ...
        if self.flash:
            if attn_mask is not None:
                # Convert bool (T,T) -> float (1,1,T,T) for SDPA
                float_mask = torch.zeros(T, T, device=x.device, dtype=x.dtype)
                float_mask = float_mask.masked_fill(~attn_mask, float('-inf'))
                y = torch.nn.functional.scaled_dot_product_attention(q, k, v, attn_mask=float_mask.unsqueeze(0).unsqueeze(0), dropout_p=self.dropout if self.training else 0, is_causal=False)
            else:
...
class GPT(nn.Module):
    ...
    def forward(self, idx=None, targets=None, inputs_embeds=None, attn_mask=None):
        if inputs_embeds is not None:
            # Hierarchical path: pre-computed embeddings, bypass wte+wpe
            b, t, _ = inputs_embeds.shape
            assert t <= self.config.block_size, f"Cannot forward sequence of length {t}, block size is only {self.config.block_size}"
            x = self.transformer.drop(inputs_embeds)
        else:
            # Original path: token ids -> wte + wpe
            device = idx.device
            b, t = idx.size()
            assert t <= self.config.block_size, f"Cannot forward sequence of length {t}, block size is only {self.config.block_size}"
