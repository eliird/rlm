...
impl Sync {
    ...
    fn check_remote_clean(server: &str, remote_path: &str) -> bool {
        let output = Command::new("ssh")
            .args([server, &format!("git -C {} status --porcelain", remote_path)])
            .output()
        ...
        if !stdout.trim().is_empty() {
            eprintln!("Remote has uncommitted changes:");
    ...
    fn check_local_clean(local_path: &PathBuf) -> bool {
        let output = Command::new("git")
            .args(["-C", local_path.to_str().unwrap(), "status", "--porcelain"])
            .output()
            .expect("Failed to run git status");
        let stdout = String::from_utf8_lossy(&output.stdout);
        if !stdout.trim().is_empty() {
            eprintln!("Local has uncommitted changes:");
            eprintln!("{}", stdout.trim());
            return false;
        }
        true
    ...
    fn check_commits_for_pull(local_path: &PathBuf, server: &str, remote_path: &str, force: bool) -> bool {
        ...
        let remote_head = match Self::get_remote_head(server, remote_path) {
            Some(h) => h,
            None => {
                eprintln!("Remote repo has no commits.");
                return false;
            }
        };
    ...
    fn run_rsync(local_path: &PathBuf, server: &str, remote_path: &str, push: bool) {
        ...
        let status = Command::new("rsync")
            .args(["-av", "--filter=:- .gitignore", "--delete", src, dst])
            .status()
    ...
    pub fn push(local_path: &PathBuf, server: &str, remote_path: &str, force: bool) {
        if !Self::check_remote_clean(server, remote_path) {
            std::process::exit(1);
        }
        if !Self::check_commits_for_push(local_path, server, remote_path, force) {
            std::process::exit(1);
        }
        Self::run_rsync(local_path, server, remote_path, true);
    }

    pub fn pull(local_path: &PathBuf, server: &str, remote_path: &str, force: bool) {
        if !Self::check_local_clean(local_path) {
            std::process::exit(1);
        }
        if !Self::check_commits_for_pull(local_path, server, remote_path, force) {
            std::process::exit(1);
        }
        Self::run_rsync(local_path, server, remote_path, false);
    }
